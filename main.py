...
from MessageCrypto import encrypt_with_password , decrypt_with_password
...
def get_title(path):
    ...
    encrypted_block = encrypt_with_password (Pass.get(), Text_Secret.get("1.0","end"))
...
def kod_cozucu():
    encrypted_block = Text_Secret.get("1.0","end").strip()
    if not Pass.get().strip() or not encrypted_block:
        messagebox.showwarning("Warning", "Please enter a valid password and valid message")
        return
    try:
        plaintext = decrypt_with_password(Pass.get(), encrypted_block)
    except Exception as e:
        messagebox.showwarning("Warning", "Parola yanlış, ya da metin bozuk")
        return
    Text_Secret.delete("1.0","end")
    Text_Secret.insert("1.0", plaintext)
...
Entry_MasterKey = tk.Entry(width=30,font=("Arial", 12,"normal"),bg="white",textvariable=Pass)
...
decrypt_button = tk.Button(text="Dectypt",font=("Arial",9,"bold"),bg="white",width=20,command=kod_cozucu)
